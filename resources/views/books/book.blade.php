...
<main class="container">
	...
	<section class="row">
		<div class="col-sm-8">
			<div class="media">
				{{-- /storage/book_images/ --}}
				<img src="{{$book->image}}" class="mr-3" alt="{{$book->title}}" width="200" height="350">
		...
		<div class="row" style='margin-top: 10px; margin-bottom: 10px;'>
			<form action="{{ route('comments.store')}}" method='POST'>
				<form action="{{ route('comments.store')}}" method='POST'>
					...
					<input type="number" min="1" max="5" name='rate' class='form-control' placeholder="rating">
			...
			<tbody>
				...
				<tr>
					...
					<td>
						...
						<form action="{{ route('comments.destroy', ['reviews'=>$storedComment->id]) }}" <form
							action="{{ route('comments.destroy', ['reviews'=>$storedComment->id]) }}" method='POST'>
							{{ csrf_field() }}
